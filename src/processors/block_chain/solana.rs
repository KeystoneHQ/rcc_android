...
pub fn process(data: Solana) -> Result<String, ParseError> {
    match data.method {
        Some(Method::ParseTransaction(data)) => {
            let message = hex::decode(data.data).map_err(|e| SolanaParseError(format!("decode hex failed")))?;
        ...
        Some(Method::VerifyMessage(data)) => {
            let mut message = hex::decode(data.data).map_err(|e| SolanaParseError(format!("decode hex failed")))?;
